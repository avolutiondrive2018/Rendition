...
<body>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            // 2. Save Image Logic (The "Nothing Happens" Fix)
            $("#gimg").off('click').on('click', async function (e) {
                e.preventDefault();
                const generateImg = document.getElementById('generateImg');
                const activeModels = document.querySelectorAll('.model-instance, #modelContainer');
                ...
                try {
                    ...
                    for (const modelGroup of activeModels) {
                        ...
                        await viewer.updateComplete;
                        const modelImg = new Image();
                        await new Promise(res => { modelImg.onload = res; modelImg.src = viewer.toDataURL('image/png'); });
            ...
            // 3. Safety Check for Original Model (only if it still exists)
            const originalModel = document.getElementById('modelContainer');
            if (originalModel) {
                initInteract(originalModel);
            }
        });

        // 4. Corrected Spawn Function
        function spawnModel(modelPath) {
            const container = document.createElement('div');
            container.className = 'model-instance'; // Class for saving
    ...
    `;
            ...
            document.getElementById('generateImg').appendChild(container);
            initInteract(container); // Hook up movement
        }

        // 5. Reusable Interact Logic
        function initInteract(element) {
            interact(element)
                .draggable({
                    allowFrom: '.move-btn',
        ...
        // Keep your background logic as is
        function readURL(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
